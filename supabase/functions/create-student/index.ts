...
serve(async (req) => {
  ...
  try {
    ...
    if (!teacherTotalErr && teacherTotal !== null && teacherTotal >= 40) {
      return new Response(
        JSON.stringify({ error: "You've reached the maximum of 40 students created. Contact an admin if you need more." }),
        { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    const { count: courseTotal, error: courseTotalErr } = await adminClient
      .from("enrollments")
      .select("id", { count: "exact", head: true })
      .eq("course_id", courseId)
      .eq("status", "active");
    console.log("Course enrollment cap check:", { courseTotal, courseTotalErr: courseTotalErr?.message });

    if (!courseTotalErr && courseTotal !== null && courseTotal >= 40) {
      return new Response(
        JSON.stringify({ error: "This course has reached the maximum of 40 enrolled students." }),
        { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
